...
# DocumentParserAgent extracts structured text from documents (PDF, images, or raw text).
#
# Usage:
#   parser = DocumentParserAgent.new
#   result = parser.parse(file_path: "doc.pdf")
#   result[:pages]       # => [{ page_number: 1, text: "...", sections: [...] }, ...]
#   result[:full_text]   # => "all text concatenated"
#   result[:page_count]  # => 3
#
#   # Or with raw text:
#   result = parser.parse(raw_text: "INTRODUCTION\nSome content...")
#
class DocumentParserAgent
  ...
  def section_heading?(line)
    ...
    return true if line.match?(/\A\d+\.\s+[A-Z]/)                   # Numbered sections
