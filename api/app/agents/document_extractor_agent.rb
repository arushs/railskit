...
# DocumentExtractorAgent pulls specific fields from document text based on a schema.
#
# Usage:
#   extractor = DocumentExtractorAgent.new
...
#     ]
#   )
#   result[:fields][:invoice_number][:value]       # => "INV-2024-001"
#   result[:fields][:invoice_number][:confidence]  # => 0.98
#   result[:fields][:invoice_number][:source_text] # => "Invoice #INV-2024-001"
#
class DocumentExtractorAgent
  ...
  def default_schema
    [
      ...
      { field_name: "date", type: "date", description: "Document date", required: false },
      { field_name: "author", type: "string", description: "Document author", required: false }
    ]
  end
end
