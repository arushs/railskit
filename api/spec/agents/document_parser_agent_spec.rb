...
RSpec.describe DocumentParserAgent do
  let(:chat) { double("chat", with_instructions: nil, ask: nil) }
  ...
  describe "#parse with raw_text" do
    ...
    it "extracts sections from ALL CAPS headings" do
      text = "OVERVIEW\nSome overview text.\n\nDETAILS\nSome detail text."
      result = agent.parse(raw_text: text)
      ...
      expect(sections.first[:content]).to include("overview text")
      expect(sections.last[:heading]).to eq("DETAILS")
    end

    it "extracts sections from numbered headings" do
      text = "1. Introduction\nSome intro text.\n\n2. Background\nSome background."
      result = agent.parse(raw_text: text)

      sections = result[:pages].first[:sections]
      expect(sections.length).to eq(2)
      expect(sections.first[:heading]).to eq("1. Introduction")
    end
    ...
    it "handles text with no headings" do
      ...
      expect(sections.first[:content]).to include("plain text")
    end
  end
  ...
  describe "#parse with image file" do
    it "uses LLM vision to extract text" do
      ...
      result = agent.parse(file_path: "/tmp/test.png")
  ...
  describe "#ask" do
    it "delegates to parse with raw_text" do
      result = agent.ask("TITLE\nSome content")
      expect(result[:full_text]).to eq("TITLE\nSome content")
    end
  end
